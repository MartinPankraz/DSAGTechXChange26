...
export function classify(input: ClassificationInput): ClassificationResult {
  ...
  if (input.isUi5) {
    return {
      ...
      priority: "LOW",
      rationale:
        "UI5 frontend code must never call the Audit Log Service directly. " +
  ...
  if (input.entityName) {
    ...
    if (
      /user|person|employee|customer|contact|payment|credit|role|auth|secret|salary|health|gdpr|personal/.test(
        lower
      )
    ) {
      return {
...
export function buildWhatToLog(
  event: string,
  entityName = "UnknownEntity",
  outcome: WhatToLog["outcome"] = "success"
): WhatToLog {
  return {
...
function importsForLibrary(library: AuditLibrary): string[] {
  switch (library) {
...
export function buildCapAuditSnippet(
  event: string,
  entityName: string,
...
export function buildCapAppLogSnippet(event: string, entityName: string): CodeSnippet {
  return {
...
export function buildPrivacyNote(
  entityName: string,
  event: string
): PrivacyNote {
  const potentiallyPersonal =
    /user|person|employee|customer|contact|salary|health/i.test(entityName);
...
export function buildValidationSteps(entityName: string, event: string): ValidationSteps {
  return {
...
export function nextSuggestionId(type: SuggestionType): string {
  _idCounter += 1;
 ...
 */
export function buildSuggestion(input: SuggestionInput): SuggestionEdit {
  const { file, event, entityName, lineHint, functionName, isUi5, auditIntegration } =
    input;
  ...
  const whatToLog = buildWhatToLog(
    event,
    entityName,
    event === "READ" || event === "EXPORT" ? "success" : "success"
  );
  ...
  return {
    ...
    correlation: {
      strategy:
        "Use x-correlation-id header if present (set by SAP BTP API Gateway / CAP framework). Fall back to crypto.randomUUID().",
 ...
 */
export function auditWrapperSource(): string {
  return `/**
