...
import { ScanWorkspaceInputSchema, SuggestLoggingInputSchema, ExplainSuggestionInputSchema, } from "./types.js";
import { runScanWorkspace } from "./tools/scan-workspace.js";
import { runSuggestLogging } from "./tools/suggest-logging.js";
import { runExplainSuggestion, registerSuggestions, } from "./tools/explain-suggestion.js";
// ─── Server definition ────────────────────────────────────────────────────────
...
server.setRequestHandler(ListToolsRequestSchema, () => {
    return {
        tools: [
            {
                name: "scan_workspace",
                description: "Detects which SAP stacks (CAP Node.js, SAPUI5) exist in the workspace and assesses " +
            ...
            },
            {
                ...
                description: "Analyses CAP (Node.js) and SAPUI5 source files and produces actionable, " +
            ...
            },
            {
                ...
                description: "Returns expanded rationale, privacy guidance, BTP verification steps, " +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (err instanceof ZodError) {
            message = `Input validation failed: ${err.errors.map((e) => `${e.path.join(".")}: ${e.message}`).join("; ")}`;
        }
...
async function main() {
    ...
    process.stderr.write("[cap-auditlog-mcp-server] Server started. Listening for MCP requests over stdio.\n");
}
main().catch((err) => {
    process.stderr.write(`[cap-auditlog-mcp-server] Fatal: ${err instanceof Error ? err.message : String(err)}\n`);
    process.exit(1);
});
