 ...
 */
export function detectAuditLibrary(files) {
    for (const file of files) {
        if (!file.path.endsWith("package.json"))
            continue;
        const deps = parseDependencies(file.content);
        if ("@cap-js/audit-logging" in deps)
            return "@cap-js/audit-logging";
        if ("@sap/audit-logging" in deps)
            return "@sap/audit-logging";
    }
    return "none";
}
 ...
 */
export function hasCdsAuditLoggingConfig(files) {
    for (const file of files) {
        ...
        if (file.path.endsWith(".cdsrc.json")) {
            if (file.content.includes("audit-logging") ||
...
export function hasMtaAuditLogResource(files) {
    for (const file of files) {
        if (file.path.endsWith("mta.yaml") || file.path.endsWith("mta.yml")) {
            ...
            if (/type:\s*org\.cloudfoundry\.existing-service/.test(file.content) &&
 ...
 */
export function hasVcapServicesAuditLog(files) {
    for (const file of files) {
        if (/VCAP_SERVICES/.test(file.content) &&
            /auditlog/.test(file.content)) {
            return true;
        }
    }
 ...
 */
export function detectIntegrationGaps(library, configuredInCds, mtaPresent, vcapUsed) {
    const gaps = [];
    if (library === "none") {
        gaps.push("No audit logging library detected in package.json. " +
            "Add @cap-js/audit-logging (preferred) or @sap/audit-logging.");
    }
    if (library === "@sap/audit-logging" && !configuredInCds) {
        gaps.push("@sap/audit-logging detected but no CDS requires configuration found. " +
            "Configure it under cds.requires.audit-log in package.json or .cdsrc.json.");
    }
    if (library === "@cap-js/audit-logging" && !configuredInCds) {
        gaps.push("@cap-js/audit-logging detected but cds.requires.audit-log not found. " +
            "Add cds.requires.auditlog = { impl: '@cap-js/audit-logging' } or enable via cds.audit-logging.");
    }
    if (!mtaPresent) {
        gaps.push("No mta.yaml auditlog service resource detected. " +
    ...
    if (!vcapUsed && !mtaPresent) {
        gaps.push("No VCAP_SERVICES auditlog binding detected and no mta.yaml resource found. " +
 ...
 */
export function detectAuditIntegration(files) {
    ...
    const capPluginActive = library === "@cap-js/audit-logging" && configuredInCdsRequires;
    const integrationGaps = detectIntegrationGaps(library, configuredInCdsRequires, mtaResourcePresent, vcapServicesUsed);
    return {
        library,
...
//# sourceMappingURL=audit-integration-detector.js.map