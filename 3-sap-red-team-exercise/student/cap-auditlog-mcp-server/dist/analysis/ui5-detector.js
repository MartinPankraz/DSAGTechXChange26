 ...
 */
export function extractUi5HandlerNames(content) {
    const found = [];
 ...
 */
export function hasODataCalls(content) {
 ...
 */
export function manifestHasODataSource(content) {
    try {
        const json = JSON.parse(content);
        const sapApp = json["sap.app"];
        if (!sapApp?.dataSources)
            return false;
        for (const ds of Object.values(sapApp.dataSources)) {
            const dsTyped = ds;
            if (typeof dsTyped.type === "string" &&
 ...
 */
export function analyseUi5Workspace(files) {
    const controllerFiles = [];
    const handlerPatternsFound = [];
    const odataCallFiles = [];
    const manifestFiles = [];
    let isUi5Workspace = false;
    for (const file of files) {
        if (!isUi5File(file.path))
            continue;
        isUi5Workspace = true;
