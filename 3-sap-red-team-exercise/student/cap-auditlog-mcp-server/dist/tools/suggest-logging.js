 ...
 */
import { analyseCapWorkspace, } from "../analysis/cap-detector.js";
import { analyseUi5Workspace, extractUi5HandlerNames, hasODataCalls, } from "../analysis/ui5-detector.js";
import { detectAuditIntegration } from "../analysis/audit-integration-detector.js";
import { auditWrapperSource, buildSuggestion, resetIdCounter, } from "../analysis/classifier.js";
import { crawlToFileNodes } from "../analysis/fs-crawler.js";
// ─── Diff parsing ─────────────────────────────────────────────────────────────
/**
 * Extracts modified file paths from a unified diff string.
 */
function extractModifiedFilesFromDiff(diff) {
    const files = [];
 ...
 */
function extractCapHandlerLocations(file) {
    const locations = [];
    const lines = file.content.split("\n");
    // Pattern A: this.on('EVENT', 'EntityName', handler)
    const withEntityRegex = /this\.(on|before|after)\s*\(\s*['"`]([A-Za-z_]+)['"`]\s*,\s*['"`]([A-Za-z_.]+)['"`]\s*,/g;
    let m;
    ...
    const actionRegex = /this\.(on|before|after)\s*\(\s*['"`]([A-Za-z_]+)['"`]\s*,\s*(?:async\s*)?\(/g;
 ...
 */
function deriveEntityFromActionName(name) {
...
function findEnclosingFunctionName(lines, targetLine) {
    for (let i = targetLine; i >= 0; i--) {
        const line = lines[i];
        if (!line)
            continue;
        const m = /(?:async\s+)?function\s+(\w+)/.exec(line) ??
...
function deriveEventFromName(name) {
    const lower = name.toLowerCase();
    if (lower.includes("create") || lower.includes("add"))
        return "CREATE";
    if (lower.includes("update") || lower.includes("save") || lower.includes("edit"))
        return "UPDATE";
 ...
 */
function extractUi5HandlerLocations(file) {
    const locations = [];
...
function extractErrorHandlerLocations(file) {
    const locations = [];
...
// ─── Coverage warnings ────────────────────────────────────────────────────────
function buildCoverageWarnings(auditIntegration, stacks) {
    const warnings = [];
    if (stacks.includes("ui5") && !stacks.includes("cap")) {
        warnings.push("Only UI5 files analysed. Audit logging suggestions point to a hypothetical CAP backend. " +
            "Provide CAP source files for precise placements.");
    }
    if (auditIntegration.library === "none") {
        warnings.push("No audit logging library installed. Suggestions include a stub wrapper – replace with a real library before production.");
    }
    if (!auditIntegration.mtaResourcePresent) {
        warnings.push("mta.yaml auditlog service binding not detected. Audit events will not reach BTP Audit Log Service at runtime.");
    }
    return warnings;
}
// ─── Main tool function ───────────────────────────────────────────────────────
export function runSuggestLogging(input) {
    resetIdCounter();
    // ── Build file nodes ─────────────────────────────────────────────────────
    ...
    const stacks = [];
    if (capResult.isCapWorkspace)
        stacks.push("cap");
    if (ui5Result.isUi5Workspace)
        stacks.push("ui5");
    ...
    for (const loc of allLocations) {
        ...
        // Apply focus filter
        if (input.focus === "audit" && suggestion.type !== "AUDIT_LOG")
            continue;
        if (input.focus === "errors" && suggestion.type !== "APP_LOG")
            continue;
        edits.push(suggestion);
        if (edits.length >= (input.maxSuggestions ?? 20))
            break;
    }
    // Sort: AUDIT_LOG HIGH first, then by priority
    const priorityOrder = { HIGH: 0, MEDIUM: 1, LOW: 2 };
    edits.sort((a, b) => {
        if (a.type === "AUDIT_LOG" && b.type !== "AUDIT_LOG")
            return -1;
        if (a.type !== "AUDIT_LOG" && b.type === "AUDIT_LOG")
            return 1;
    ...
    if (wrapperNeeded) {
        coverageWarnings.push("A stub audit log wrapper will be referenced at ./lib/audit-log-wrapper.js – create this file using the auditWrapperSource() helper or replace with a real library.");
    }
    return {
        summary: {
...
function deriveEntityFromDiffPath(p) {
...
//# sourceMappingURL=suggest-logging.js.map