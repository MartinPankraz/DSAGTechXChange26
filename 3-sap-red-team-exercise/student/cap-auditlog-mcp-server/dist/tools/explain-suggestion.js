...
export function registerSuggestions(edits) {
...
function buildExpandedRationale(edit) {
    const parts = [];
    ...
    if (edit.type === "AUDIT_LOG") {
        ...
        parts.push("Under GDPR (EU) / DSGVO (Germany), organisations must be able to demonstrate " +
        ...
        parts.push("This location does not handle personal data mutation or security-relevant decisions. " +
...
function buildPrivacyGuidance(edit) {
    const parts = [];
...
function buildAlternativePlacements(edit) {
    const alts = [];
    if (edit.type === "AUDIT_LOG") {
        ...
        alts.push({
            ...
            tradeoff: "Logging in a before() handler fires even when the operation fails. " +
        ...
        alts.push({
            ...
            tradeoff: "Logging in an after() handler fires only on success. " +
        ...
        if (edit.ui5BackendNote) {
            ...
            alts.push({
                ...
                tradeoff: "Primary recommended location – audit logging in the CAP handler that " +
        ...
        alts.push({
            ...
            tradeoff: "Add the log call at module level if you want startup / initialisation diagnostics.",
        });
    }
    return alts;
}
// ─── Main tool function ───────────────────────────────────────────────────────
export function runExplainSuggestion(input) {
    ...
    if (!edit) {
        ...
        return {
            ...
            privacyGuidance: "No suggestion found. Run suggest_logging to generate suggestion IDs.",
    ...
    const references = edit.type === "AUDIT_LOG"
...
//# sourceMappingURL=explain-suggestion.js.map