 ...
 */
import { analyseCapWorkspace, buildCapEvidence, } from "../analysis/cap-detector.js";
import { analyseUi5Workspace, buildUi5Evidence, } from "../analysis/ui5-detector.js";
...
function buildRecommendedNextSteps(ctx) {
    const steps = [];
    const { stacks, auditIntegration } = ctx;
    if (!stacks.includes("cap") && !stacks.includes("ui5")) {
        steps.push("No SAP CAP or UI5 workspace detected. " +
    ...
    if (stacks.includes("cap")) {
        ...
        if (auditIntegration.library === "none") {
            steps.push("ACTION REQUIRED: Install @cap-js/audit-logging → npm install @cap-js/audit-logging");
            steps.push("ACTION REQUIRED: Add CDS plugin config to package.json:\n" +
                '  "cds": { "requires": { "audit-log": { "impl": "@cap-js/audit-logging" } } }');
        }
        if (!auditIntegration.mtaResourcePresent) {
            steps.push("Add auditlog managed service resource to mta.yaml and bind it to your app module.");
        }
        if (!auditIntegration.configuredInCdsRequires && auditIntegration.library !== "none") {
            steps.push("Configure audit-log in cds.requires section of package.json or .cdsrc.json.");
        }
    }
    if (stacks.includes("ui5")) {
        steps.push("UI5 workspace detected. Note: audit logging must NEVER be done from UI5 frontend code. " +
            "All audit events must be emitted in CAP backend handlers.");
        if (!stacks.includes("cap")) {
            steps.push("No companion CAP backend detected in this workspace. " +
...
// ─── Main tool function ───────────────────────────────────────────────────────
export function runScanWorkspace(input) {
...
//# sourceMappingURL=scan-workspace.js.map